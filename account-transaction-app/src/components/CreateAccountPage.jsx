...
class CreateAccountPage extends React.Component {
    ...
    handleValidateForm = () => {
        const { isEmailValid, isMobileValid, isAccountExist, isMobileNoExist } = this.state;
        if (!isEmailValid || !isMobileValid || isAccountExist || isMobileNoExist) return false;
        return true;
    };

    handleRequiredFields = () => {
        const { firstName, lastName, mobileNo, email, balance } = this.state;
        if (!firstName || !lastName || !mobileNo || !balance || !email) return false;
        return true;
    };
    ...
    handleSubmit = async event => {
        ...
        const { firstName, lastName, mobileNo, email, balance,accountNo } = this.state;
    ...
    handleIsAccountExist = async () => {
        ...
        const response = await http.get(`${this.baseUrl}/check/account/${accountNo}`);
    ...
    handleExistMobileNo = async () => {
        ...
        const response = await http.get(`${this.baseUrl}/checkmobileNo/${mobileNo}`);
        if (response.status === 200) {
            console.log(response.data);
    ...
    render() {
        const { firstName, lastName, mobileNo, email,accountNo, balance, isEmailValid, isMobileValid, isAccountExist, isMobileNoExist } = this.state;
        return (
                    ...
                    <form onSubmit={this.handleSubmit}>
                        ...
                        <div className="form-group row">
                            ...
                            <div className="col-sm-8">
                                ...
                                />
                                {isMobileNoExist && (<span className="text-danger">Duplicate Mobile No. is Already Exist! Try Again.</span>)}
                                {isMobileValid === false && (<span className="text-danger">Invalid Mobile No. (like-01****79339)</span>)}
                            </div>
                        </div>
                        <div className="form-group row">
                            <label htmlFor="mobileNo" className="col-sm-4 col-form-label">
                            ...
                            <div className="col-sm-8">
                                ...
                                />
                                {isEmailValid === false && (<span className="text-danger">Invalid Email.(Ex - name@gmail.com)</span>)}
                            </div>
                        </div>
                        <div className="form-group row">
